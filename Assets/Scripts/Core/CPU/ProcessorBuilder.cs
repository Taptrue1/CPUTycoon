...
namespace Core.CPU
{
    public class ProcessorBuilder : MonoBehaviour
    {
        ...
        private float _frequencyChangeStep;
        ...
        private void Awake()
        {
            ...
            _currentFrequency = new CustomNumber<float>() { Value = _frequencyBounds.Min };
            _frequencyChangeStep = (_frequencyBounds.Max - _frequencyBounds.Min) / 10;
        ...
        {
            _currentFrequency.Value = Mathf.Clamp(_currentFrequency.Value + _frequencyChangeStep, _frequencyBounds.Min, _frequencyBounds.Max);
        }
